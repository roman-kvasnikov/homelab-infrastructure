...
Домашний Traefik (`192.168.40.11`, DMZ) — второй конец туннеля и точка терминации TLS. Детали Traefik (домены, сертификаты, middleware) — в `traefik.md`; здесь только то, что относится к стыку с VPS.
...
Основной домен — `kvasok.xyz`, wildcard `*.kvasok.xyz` указывает на публичный адрес VPS. Сертификаты Let's Encrypt получает **Traefik** через DNS-01 challenge (wildcard). VPS сертификаты не хранит и в выдаче не участвует — он работает на L4, не видя TLS. Детали ACME — в `traefik.md`.
