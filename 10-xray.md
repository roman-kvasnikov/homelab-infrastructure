...
name: xray
description: |
  Xray-прокси для гео-обхода: VM в INFRA. Два режима — forward-прокси (SOCKS/HTTP) для явных потребителей (медиастек в SERVICES) и прозрачное проксирование телевизоров через REDIRECT + policy-routing на OPNsense. Документ описывает конфиг Xray, nftables, sysctl, автозапуск и связку с OPNsense. Используй для вопросов по гео-обходу, проксированию телевизоров, медиастека.
---
...
Второй потребитель — Traefik-LXC и CrowdSec-движок в DMZ (`192.168.40.11`): их исходящий трафик завёрнут через `HTTP_PROXY`/`HTTPS_PROXY` на `http://192.168.20.12:10809` в systemd-юнитах `traefik.service` и `crowdsec.service` — для скачивания плагина CrowdSec, выпуска сертификатов через `namecheap` (DNS-01 ACME) и синхронизации с CAPI. Это межсегментный доступ DMZ → INFRA (детали — в `08-traefik.md`).
...
- **Traefik / CrowdSec (`192.168.40.11`)** — потребитель forward-прокси (`10809`) для скачивания плагина, namecheap-ACME и CAPI-синхронизации (см. `08-traefik.md`).
