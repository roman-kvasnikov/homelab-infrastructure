...
Сеть контейнера — тегированный VLAN-интерфейс на VLAN-aware мосту гипервизора (тег назначается по роли сервиса согласно схеме VLAN в `home-network.md`). Адрес статический либо резервируется в Kea DHCP по MAC; шлюз и DNS — адрес OPNsense в соответствующем VLAN (`192.168.<vlan>.1`).
