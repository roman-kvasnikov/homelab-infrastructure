...
| `backup@pbs` | `pve` | `/datastore/Homelab` | `DatastoreBackup` | бэкапы с PVE-Main |
...
`backup@pbs` отвечает только за запись бэкапов — под ним два токена, `pve` (PVE-Main) и `pve-mini` (PVE-Mini). `monitoring@pbs` отвечает только за read-only доступ мониторинга — токены `homepage` (виджет Homepage) и `pbs-exporter` (стек Prometheus), оба с ролью `Audit`.
...
- **Гипервизоры (PVE-Main, PVE-Mini)** — источники host-backup и PBS-снапшотов через токены `backup@pbs!pve` / `pve-mini`.
