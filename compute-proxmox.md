---
name: compute-proxmox
description: |
  Вычислительный слой homelab: два хоста Proxmox VE. PVE-Mini несёт сетевую плоскость (OPNsense, Omada, AmneziaWG, Xray), PVE-Main — прикладные сервисы. Документ описывает сеть хостов, storage и ZFS, лимиты ресурсов гостей, UPS/NUT и инвентарь VM/LXC.
---

# Гипервизоры Proxmox

Вычислительная нагрузка распределена на два хоста Proxmox VE. Разделение по роли: **PVE-Mini** несёт сетевую плоскость (маршрутизатор, контроллер коммутатора, VPN, прокси), **PVE-Main** — прикладные сервисы (reverse-proxy, приложения, мониторинг). Оба хоста управляются в MGMT (VLAN 10); все гости получают VLAN тегированием на виртуальных интерфейсах.

Общие соглашения по LXC (unprivileged + `nesting=1`, sandbox, nftables) вынесены в `conventions.md` и здесь не повторяются.
...
**Проброс дисков.** Физические диски ZFS-пулов проброшены в VM. На каждом проброшенном диске стоит флаг **`backup=0`** — PBS не пытается бэкапить многотерабайтные ZFS-пулы, снимается снапшот только системного диска VM. Данные пулов бэкапятся отдельно через restic (см. `backup.md`) либо не бэкапятся вовсе (медиатека, записи камер).

**iGPU passthrough.** В DockerHost проброшен Intel iGPU (`vfio-pci`) для аппаратного транскодинга Jellyfin. Детали Docker-стека и ZFS-датасетов — в `services-dockerhost.md`.
...
DockerHost получает основную долю RAM с включённым balloon; остальные гости — фиксированные скромные лимиты. ZFS ARC на PVE-Main ограничен 4 GB (см. раздел 3), чтобы кэш файловой системы не конкурировал с памятью гостей.
