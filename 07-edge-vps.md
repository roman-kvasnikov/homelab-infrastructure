...
Этот путь отвечает **только** за публикацию HTTP-сервисов наружу.
...
Внешний Linux-VPS — единственная точка входа публичного трафика. Firewall — nftables. Сертификаты не хранит, TLS не терминирует. Держит служебный WireGuard-туннель к домашнему Traefik.
...
Служебный туннель между VPS (`10.0.0.1`) и Traefik (`10.0.0.2`). Инициирует соединение **домашний Traefik** (исходящее), поэтому на домашнем роутере не нужен проброс портов — туннель поднимается изнутри наружу. По туннелю идёт только публичный HTTPS-трафик (проксируемый nginx'ом на VPS в сторону Traefik).

## 3. Сторона Traefik

Домашний Traefik (`192.168.40.11`, DMZ) — второй конец туннеля и точка терминации TLS. Детали Traefik (домены, сертификаты, middleware) — в `07-traefik.md`; здесь только то, что относится к стыку с VPS.
...
Основной домен — `kvasok.xyz`, wildcard `*.kvasok.xyz` указывает на публичный адрес VPS. Сертификаты Let's Encrypt получает **Traefik** через DNS-01 challenge (wildcard). VPS сертификаты не хранит и в выдаче не участвует — он работает на L4, не видя TLS. Детали ACME — в `07-traefik.md`.

Внутри сети те же имена резолвятся split-horizon (Unbound) в локальный адрес Traefik (`192.168.40.11`), минуя VPS — см. `03-network.md`.

## 5. Ключевые особенности

- **VPS — только L4-вход, ничего больше.** Nginx stream проксирует 443 в туннель с PROXY protocol. TLS не терминирует, сертификаты не хранит. Минимальная поверхность: по сути перекладчик пакетов.
