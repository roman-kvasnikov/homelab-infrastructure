...
Доступ из IoT-сегмента ограничен ровно двумя конкретными хостами в основном сегменте — никаких разрешений на «весь 192.168.1.0/24». На уровне Traefik доступ из IoT дополнительно ограничен только сервисом Jellyfin через middleware-цепочку `internal-chain-with-iot` (см. раздел 6.5).
...
| `trusted-and-iot`             | ipAllowList: LAN, VPN и `192.168.10.0/24` (IoT) — расширение `trusted`      |
...
- **`internal-chain-with-iot`** — расширение `internal-chain` на IoT-сегмент. Применяется только к Jellyfin, чтобы телевизоры из IoT могли стримить с него, но не имели доступа ни к каким другим внутренним сервисам.
...
| LAN (192.168.1.0/24) и IoT (192.168.10.0/24) на `eth0` | TCP 443 | HTTPS из LAN и IoT (через split-horizon DNS возвращающий 192.168.1.15) |
...
table inet filter {
    chain input {
        ...
        # HTTPS из LAN и IoT (split-horizon DNS возвращает 192.168.1.15)
