...
Vaultwarden бэкапится **в отдельный restic-репозиторий**. Это даёт изоляцию: компрометация Vaultwarden LXC не позволяет атакующему добраться до других бэкапов.
...
- **Backrest** делает SFTP-бэкапы на `192.168.1.11`.
...
Бэкапы организованы по **четырёхуровневой схеме**, где каждый уровень закрывает свою задачу и использует подходящий для неё инструмент. Все бэкапы складываются на один **бэкап-сервер 192.168.1.11**, но в разные структуры на одном RAID0 массиве.

### 12.1. Бэкап-сервер (192.168.1.11)

ОС — **Proxmox Backup Server** (поверх Debian 12). Помимо самого PBS, на сервере поднят **SSH/SFTP-доступ** для приёма restic-бэкапов с Vaultwarden LXC, Authelia LXC и DockerHost VM.
...
### 12.4. Уровень 3: Данные Vaultwarden и Authelia через restic

PBS-снапшот всего LXC закрывает сценарий «снёс LXC целиком, нужно восстановить за минуту». Но для Vaultwarden и Authelia этого мало: при компрометации или повреждении БД нужна возможность достать конкретную версию SQLite-файла от N дней назад без подъёма целого LXC. Поэтому поверх PBS-снапшотов оба сервиса делают независимый restic-бэкап своих данных.

#### Vaultwarden (192.168.1.17)

**Что бэкапится:** `/var/lib/vaultwarden/data/` — БД (через online-снапшот `sqlite3 .backup`), `attachments/`, `rsa_key.pem`. Исключаются живой `db.sqlite3`, WAL-файлы (`-shm`, `-wal`), `icon_cache/` и `tmp/`.

**Чем:** скрипт `/usr/local/sbin/vaultwarden-backup.sh`, запускаемый systemd-таймером `vaultwarden-backup.timer`. Скрипт делает три шага: атомарный снапшот SQLite через `sqlite3 .backup`, `restic backup` с тегом `vaultwarden` и host `vaultwarden`, затем `restic forget --group-by host,tags --prune` с retention 7 daily / 4 weekly / 6 monthly. Группировка по host+tags гарантирует, что все снимки попадают в одну группу retention независимо от изменения путей в скрипте.

**Куда:** SFTP на бэкап-сервер, репозиторий `/mnt/data/restic/vaultwarden/`.

**Восстановление БД:** после `restic restore` файл лежит как `db.sqlite3.backup` — нужно переименовать в `db.sqlite3` перед запуском Vaultwarden. WAL-файлы при восстановлении не нужны и автоматически создадутся при первом старте.

#### Authelia (192.168.1.18)

**Что бэкапится:** `/var/lib/authelia/` (БД через online-снапшот, `notifier.log`) и `/etc/authelia/` (все YAML-конфиги, секреты в `/etc/authelia/secrets/`). Исключаются живой `db.sqlite3`, WAL-файлы, `.ssh/`, `.restic-password` (chicken-and-egg) и `.cache/`.

**Чем:** скрипт `/usr/local/sbin/authelia-backup.sh`, запускаемый systemd-таймером `authelia-backup.timer`. Логика аналогична Vaultwarden: `sqlite3 .backup` → `restic backup` (тег `authelia`, host `authelia`) → `restic forget --group-by host,tags --prune` с retention 7 daily / 4 weekly / 6 monthly.

**Куда:** SFTP на бэкап-сервер, репозиторий `/mnt/data/restic/authelia/`.

**Восстановление БД:** аналогично Vaultwarden — переименовать `db.sqlite3.backup` в `db.sqlite3` перед стартом.

#### Изоляция SSH-доступа к бэкап-серверу

Каждый из двух сервисов (а также DockerHost из уровня 4) ходит на бэкап-сервер под **отдельным юзером** — `backup-vaultwarden`, `backup-authelia`, `backup-dockerhost`. Это даёт изоляцию: компрометация одного LXC не позволяет атакующему добраться до бэкапов других сервисов.

Каждый юзер настроен через `Match User` в `/etc/ssh/sshd_config` бэкап-сервера: `ChrootDirectory` в свою home-директорию, `ForceCommand internal-sftp` (никакого shell), shell `/usr/sbin/nologin`, отключены `AllowTcpForwarding`, `X11Forwarding`, `PermitTunnel`. Каталоги вида `/mnt/data/restic/<service>/` физически живут на RAID0-массиве, но доступны внутри chroot через bind mount в `/home/backup-<service>/restic/` (запись в `/etc/fstab`).

Аутентификация — по ed25519-ключу (`/var/lib/<service>/.ssh/id_backup`), пароль restic-репозитория хранится в `/var/lib/<service>/.restic-password` (chmod 600, owner соответствующего системного юзера) и продублирован в офлайн-хранилище для disaster recovery.

### 12.5. Уровень 4: Данные внутри DockerHost через restic
...
- `/mnt/data/` — все volumes сервисов в Docker и SAMBA shares.
...
#### Restic для Vaultwarden и Authelia

Единая retention-политика, заданная прямо в скриптах бэкапа: **7 daily / 4 weekly / 6 monthly** + `--prune`. Группировка `--group-by host,tags`.

#### Restic для DockerHost
...
9. При необходимости восстановить отдельные файлы AppData из restic, БД Vaultwarden из `/mnt/data/restic/vaultwarden/`, БД и конфиги Authelia из `/mnt/data/restic/authelia/`
