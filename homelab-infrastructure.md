...
## 2. Storage на Proxmox host

На самом pve-хосте развёрнуто три storage:
...
У Xray `up=30` — Traefik при старте качает плагин CrowdSec через `HTTP_PROXY=http://192.168.1.3:10809` (Xray-Proxy), и нужна задержка, чтобы Xray успел поднять прокси-порт. Без задержки Traefik стартует с невалидной конфигурацией (плагин не загружается, все роутеры с middleware `crowdsec` становятся невалидными → 404 на всех сервисах). Симптом — после ребута pve все публичные сервисы возвращают «404 page not found», лечится `systemctl restart traefik` в LXC.

## 2. Домены и сертификаты
...
### 3.1. Основной сегмент LAN (192.168.1.0/24)
...
### 3.2. Сегмент Cameras (192.168.30.0/24)
...
- Сертификаты получает сам через DNS Challenge — TLS-резолвер `timewebcloud`, для отдельных хостов также `namecheap`.
...
### 6.5. Доверие к заголовкам и trusted IPs
...
На pve-хосте установлен и настроен **NUT** (Network UPS Tools) для корректного
обращения с UPS при отключении электричества.

### 11.1. Конфигурация

Конфиги: `/etc/nut/` (бэкапится в PBS host backup, см. 10.3).

### 11.2. Поведение при пропадании питания

При срабатывании события `low battery` NUT инициирует graceful shutdown гостей в обратном порядке startup (см. таблицу 2.3): сначала DockerHost (с downtime до 180 с на корректное завершение Docker-контейнеров), последним — AdGuard и Xray. Это даёт максимум шансов корректно сохранить состояние БД и не потерять данные.
