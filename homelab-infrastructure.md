...
### 2.2. Сегмент IoT (192.168.10.0/24)

Отдельный изолированный сегмент для IoT-устройств (телевизоры, умные лампочки и т.п.) — отдельный SSID `Izolda-Rally-IoT` (2.4 ГГц, WPA2), привязанный к этому сегменту в Keenetic. Роутер сегмента — `192.168.10.1`. DHCP-пул `192.168.10.100–199`. IP адреса определяются динамически. В DHCP-настройках сегмента DNS-сервер явно указан как `192.168.1.2` (AdGuard) — теоретически это нужно, чтобы AdGuard видел в логах реального клиента (телевизор, лампочку), а не сам Keenetic. На практике per-client логи не работают из-за SNAT (см. ниже).

#### Permit-правила

Заведены на интерфейсе **«IoT»** (трафик инициируют устройства IoT):

| Действие   | Источник         | Назначение     | Порт         | Назначение                  |
|------------|------------------|----------------|--------------|-----------------------------|
| Разрешить  | 192.168.10.0/24  | 192.168.1.2    | UDP+TCP 53   | DNS через AdGuard           |
| Разрешить  | 192.168.10.0/24  | 192.168.1.15   | TCP 443      | HTTPS до Traefik (Jellyfin) |

Доступ из IoT-сегмента физически ограничен ровно двумя конкретными хостами в основном сегменте — никаких разрешений на «весь 192.168.1.0/24».

#### Особенность SNAT в Keenetic

Keenetic при межсегментном роутинге применяет SNAT — Traefik (и AdGuard) видят source IP всех IoT-устройств как `192.168.1.1` (адрес роутера), а не как реальный `192.168.10.X`. Это поведение управляется единственной опцией «Использовать NAT» в настройках сегмента (CLI: `ip nat Bridge2`), которая в прошивке Keenetic 2.06.1 (KN-1012) **не разделяется** на «NAT для интернета» и «NAT для межсегментного трафика». Селективно отключить SNAT только для межсегментного направления нельзя ни через UI, ни через CLI — расширенная форма `ip nat <source> to <destination>` в этой прошивке не поддерживается. Если выключить опцию целиком, в IoT пропадает интернет (приватные адреса не маршрутизируются провайдером без NAT на WAN).

Практические последствия:

- **Фильтрация IoT по source IP в Traefik бесполезна** — IoT-трафик неотличим от LAN-трафика, попадает в middleware `trusted` (так как `192.168.1.1` входит в `192.168.1.0/24`).
- **Per-client логи в AdGuard для IoT не работают** — все DNS-запросы выглядят как идущие от `192.168.1.1`.
- **Логически IoT всё равно изолирован** — пакеты физически могут уйти только в два разрешённых хоста (AdGuard и Traefik), permit-правила в Keenetic действуют до SNAT.

Защита от компрометированных IoT-устройств строится на других уровнях, а не на фильтрации в Traefik:

1. Permit-правила Keenetic дают IoT доступ только к `192.168.1.2:53` и `192.168.1.15:443` — никуда больше в основной сегмент попасть нельзя.
2. Authelia защищает чувствительные сервисы (Vaultwarden, Proxmox UI, dashboard) — IoT-устройство не пройдёт MFA, даже если резолвит и достучится до Traefik.
3. Состав сервисов в Traefik контролируется явно — посторонних сервисов там нет.

Если в будущем мигрировать роутер на платформу с гранулярным контролем NAT (OPNsense/pfSense/MikroTik), эту главу можно будет переписать в исходном виде: отключить SNAT только для межсегментного трафика, вернуть `trusted-and-iot` и `internal-chain-with-iot`, получить per-client логи в AdGuard.

### 2.3. Сегмент Surveillance (192.168.20.0/24)

Отдельный изолированный сегмент для IP-камер — отдельный SSID `Izolda-Rally-Surveillance` (2.4 ГГц, WPA2), привязанный к этому сегменту в Keenetic. Роутер сегмента — `192.168.20.1`. DHCP-пул `192.168.20.100–199`, камерам зарезервированы статические адреса по MAC.

| IP             | Хост                  | Роль                                                   |
|----------------|-----------------------|--------------------------------------------------------|
| 192.168.20.101  | Tapo C320WS          | Уличная камера (RTSP)                                  |
| 192.168.20.102  | Tapo C220            | Камера в прихожей (RTSP + ONVIF)                       |

#### Изоляция сегментов

На уровне Keenetic включён `isolate-private` — между сегментами запрещено всё, что не разрешено явно.
...
| Действие   | Источник       | Назначение         | Порт     | Назначение            |
|------------|----------------|--------------------|----------|-----------------------|
| Разрешить  | 192.168.1.20   | 192.168.20.0/24    | TCP 554  | RTSP для Frigate      |
| Разрешить  | 192.168.1.20   | 192.168.20.0/24    | TCP 2020 | ONVIF для Frigate     |

В сегменте Surveillance также включён NAT для возможности обновления прошивок камер и работы Tapo-приложения (push-уведомления и т.п.). Если в будущем захочется отрезать камеры от интернета полностью — это делается через «Политику доступа» сегмента, а не через firewall-правила.
...
└── zguests (ZFS pool, lz4, ashift=12, volblocksize=16k)
    ├── system-диски LXC AdGuard, Xray, Traefik, Vaultwarden, Authelia, Gotify, Monitoring
    ├── system-диск DockerHost VM (200 GB)
...
| Gotify (116)      | 512 MB / 512 swap | 1     | order=3, up=10, down=30   |
| Vaultwarden (117) | 512 MB / 512 swap | 1     | order=3, up=5, down=30    |
| Authelia (118)    | 512 MB / 512 swap | 1     | order=3, up=10, down=30   |
| Monitoring (119)  | 2 GB / 512 swap   | 2     | order=3, up=10, down=30   |
| DockerHost (120)  | 32G / 24G balloon | 4     | order=10, up=60, down=180 |
...
Конфиги: `/etc/nut/` (бэкапится в PBS host backup, см. 12.2).
...
- `minVersion: VersionTLS12` — TLS 1.3 рассматривался как минимум, но WebOS на LG TV (актуально для подключённого `TV - LG` 44:cb:8b:6a:19:64) не поддерживает TLS 1.3, и Jellyfin на нём при TLS 1.3 падает на этапе handshake без записи в access log. TLS 1.2 с современными cipher suites остаётся приемлемым уровнем безопасности.
...
| `trusted`                     | ipAllowList: `192.168.1.0/24` (LAN) и `10.8.0.0/24` (VPN)                   |
...
| `security-headers-browser`    | referrer policy (strict-origin-when-cross-origin), permissions policy (запрет camera/microphone/geolocation/USB/Bluetooth и т.д.), CSP (`frame-ancestors 'self'`, `base-uri 'self'`, `form-action 'self'`) — заголовки, имеющие смысл только для браузерных клиентов |
...
- **`internal-chain`** — только LAN и VPN. Применяется чисто к внутренним сервисам: Traefik dashboard, Proxmox UI, AdGuard, Portainer, pgAdmin и т.д.
...
| LAN (192.168.1.0/24) на `eth0`    | TCP 443         | HTTPS из LAN (через split-horizon DNS возвращающий 192.168.1.15). IoT-трафик тоже разрешается этим же правилом — после SNAT в Keenetic он приходит с source `192.168.1.1`, попадающим в `LAN_NET`. |
...
Default address pool в `/etc/docker/daemon.json` ограничен диапазоном `10.200.0.0/16` (size `/24`), чтобы Docker автоматически создаваемые сети не пересекались с физическими подсетями LAN/Surveillance/VPN:
...
Основной IdP — **Authelia**, развёрнут в отдельном LXC (`192.168.1.18`, см. раздел про **Authelia LXC**). Конфигурация разнесена по YAML-файлам в `/etc/authelia/`. 2FA через TOTP и WebAuthn, поддержка PKCE для мобильных приложений. Forward-auth делается middleware `authelia` в Traefik (`http://192.168.1.18:9091/api/authz/forward-auth`) — любой защищённый сервис получает `internal-chain + authelia` или `external-chain + authelia`.
...
| `monitoring@pbs!pbs-exporter` | `/`                | 1         | `Audit`           |
...
User-account `pve-backup@pbs` отвечает только за запись бэкапов. Под ним два токена: `pve-main` для бэкапов VM/LXC через pve storage (см. раздел 12.3), и `pve-host` для host backup конфигов pve (см. раздел 12.2). User-account `monitoring@pbs` отвечает только за read-only доступ для интеграций мониторинга. Под ним два токена: `homepage` — для виджета PBS в homepage dashboard, и `pbs-exporter` — для pbs-exporter в стеке Prometheus (см. раздел 14). Оба с ролью `Audit`.
...
- `PasswordAuthentication no` — парольная аутентификация полностью отключена.
- `KbdInteractiveAuthentication no` — закрыт обходной путь через keyboard-interactive.
- `X11Forwarding no` — на headless сервере не нужен.
- `AllowAgentForwarding no` — закрыт типичный путь lateral movement через ssh-agent.
- `AllowTcpForwarding no` — PBS нельзя использовать как туннельный прокси через `ssh -L`.
...
## 13. Gotify LXC (192.168.1.16)

Сервер push-уведомлений, вынесен в отдельный LXC на основном Proxmox-хосте. Работает нативно (без Docker) на Go-бинарнике Gotify v2.9.1 под systemd. База данных — SQLite. Источник уведомлений для алертов мониторинга и прочих сервисов: каждое приложение-источник имеет свой токен и шлёт сообщения в Gotify, а клиенты (телефон, браузер) их принимают.

### 13.1 Файловая структура

```
/usr/local/bin/gotify                  # бинарь, скачан с github.com/gotify/server/releases
/etc/gotify/config.yml                 # конфигурация
/etc/systemd/system/gotify.service
/var/lib/gotify/                        # домашняя директория системного юзера gotify
└── data/
    └── gotify.db                       # основная БД (SQLite): пользователи, приложения, сообщения
```

### 13.2 Systemd

Сервис `gotify.service` запускает бинарник от выделенного системного юзера `gotify`. Включён sandbox-набор по образцу Vaultwarden/Authelia: `NoNewPrivileges`, `ProtectSystem=strict`, `ProtectHome`, `PrivateTmp`, `PrivateDevices`, `ProtectKernel*`, `RestrictNamespaces`, `LockPersonality`. Запись разрешена только в `/var/lib/gotify` через `ReadWritePaths`. Перезапуск при падении — `Restart=always`. Автозапуск при старте контейнера (`enabled`).

### 13.3 Конфигурация (`/etc/gotify/config.yml`)

Gotify слушает только на конкретном LAN-IP — `192.168.1.16:8060`, не на `0.0.0.0`, по аналогии с Vaultwarden и Authelia. Прямой доступ к порту ограничивается дополнительно на уровне nftables (см. ниже).

### 13.4 Сетевая фильтрация (nftables)

Gotify принимает входящие соединения на `192.168.1.16:8060` **только** с адреса Traefik (192.168.1.15). Все остальные источники в LAN и из VPN получают `drop` — соединение не устанавливается. Это закрывает прямой доступ к Gotify мимо Traefik с его middleware-цепочкой. Подход — whitelist с `policy drop`, идентичный шаблону Vaultwarden/Authelia, отличается только портом (8060).
...
table inet filter {
    chain input {
        ...
        # Gotify only from Traefik
        tcp dport 8060 ip saddr $TRAEFIK_IP accept
...
### 13.5 Публикация и авторизация

Gotify опубликован через Traefik как `gotify.kvasok.xyz` под цепочкой `external-chain` (CrowdSec + rate-limit + security-headers), **без** middleware `authelia`. Это сознательное исключение из общего правила раздела 11: forward-auth Authelia несовместим с родной моделью авторизации Gotify. Gotify использует собственную авторизацию (логин/пароль пользователя для веб-интерфейса и приложений, токен приложения для отправки сообщений) и Basic Auth для ряда эндпоинтов; при включённом Authelia forward-auth запросы к `/manifest.json` и к API заворачиваются на страницу логина Authelia ещё до того, как сработает родная авторизация Gotify, и клиент не может пройти аутентификацию. Поэтому Gotify выведен из-под Authelia и защищён только собственной авторизацией.

> ПОМЕТКА ДЛЯ ТЕБЯ: если в итоге ты выбрал `external-chain-strict` (жёсткий rate-limit, обсуждали как вариант, потому что наружу сервис под паролем без MFA) — поправь название цепочки здесь на `external-chain-strict`. По моим данным финальное состояние — обычный `external-chain`.

### 13.6 Резервное копирование

Только PBS-снапшот всего LXC в составе общего ежедневного pve-задания. Отдельный restic осознанно не заводился — по той же логике, что и для Traefik LXC: критичного point-in-time состояния, ради которого нужен гранулярный restic (как БД с паролями у Vaultwarden или с TOTP/OIDC у Authelia), у Gotify нет; SQLite с подписками и историей сообщений некритична, а file-level restore одного файла из CT-снапшота PBS при необходимости доступен.

### 13.7 Зависимости от внешних сервисов

- **Traefik LXC (192.168.1.15)** — единственный разрешённый источник запросов к 8060 (через nftables). Без Traefik сервис недоступен снаружи LXC.
...
CTID `104`, hostname `monitoring`, IP `192.168.1.19/24`, gw `192.168.1.1`, Debian 13 (Trixie), unprivileged, `nesting=1` (только под systemd 257, как у Traefik LXC; без `keyctl`/`mount`/`mknod`). Ресурсы: 2 GB RAM / 512 swap, 2 cores, диск 16 GB на `zguests`. Startup `order=3` (одна волна с Vaultwarden/Authelia). DNS — AdGuard (192.168.1.2). sshd ужесточён через тот же drop-in `/etc/ssh/sshd_config.d/10-hardening.conf`, что и на остальных хостах.

### 14.2 Компоненты на самом LXC

- **Prometheus 3.12.0** — Go-бинарь под выделенным системным юзером с systemd-sandbox, слушает `:9090`. Хранит TSDB, скрейпит все экспортеры ниже.
- **Grafana** — из официального apt-репозитория, слушает `:3000`. Datasource Prometheus провижинится файлом `/etc/grafana/provisioning/datasources/prometheus.yml`. `root_url = https://grafana.kvasok.xyz/`, `cookie_secure = true`.
- **node_exporter** — на самом monitoring LXC, забинден на loopback.

### 14.3 Экспортеры по другим хостам

- **node_exporter на pve-хосте (192.168.1.10)** — забинден на LAN, запущен с `--collector.zfs` и `--collector.processes`. Доступен Prometheus напрямую, потому что Proxmox Firewall на хосте сейчас не активирован (его полноценная активация отложена до после миграции на OPNsense).
- **node_exporter на бэкап-сервере (192.168.1.11)** — плюс textfile-collector, читающий `.prom`-файлы из `/var/lib/node_exporter/textfile`.
- **pve-exporter** — Python/gunicorn в venv `/opt/pve-exporter`, отдаёт статус и метрики всех VM/LXC. Ходит в Proxmox API под токеном `monitoring@pve` с ролью `PVEAuditor` и `privsep=0`.
...
Grafana опубликована через Traefik как `grafana.kvasok.xyz` под собственной авторизацией Grafana. Напрямую из доверенной сети Grafana (`:3000`) и Prometheus (`:9090`) тоже доступны — см. firewall ниже.

### 14.6 Сетевая фильтрация (nftables)

Whitelist с `policy drop` на `input`/`forward`. В отличие от Vaultwarden/Authelia/Gotify (где порт сервиса открыт только с Traefik), здесь открыты порты 22 (SSH), 3000 (Grafana) и 9090 (Prometheus) для всей доверенной сети — LAN `192.168.1.0/24` и VPN `10.8.0.0/24`. Это сознательно: к Grafana и Prometheus удобно ходить напрямую из доверенной сети, а сами по себе они под собственной авторизацией Grafana и закрытым наружу Prometheus.
...
table inet filter {
    chain input {
        ...
        # SSH, Grafana, Prometheus from TRUSTED_NET and VPN_NET
        tcp dport { 22, 3000, 9090 } ip saddr { $TRUSTED_NET, $VPN_NET } accept
...
### 14.7 Резервное копирование

Только PBS-снапшот всего LXC (selection `All` в общем pve-задании). Отдельный restic не заводился: дашборды и провижининг Grafana хранятся в `grafana.db` и файлах провижининга и восстанавливаются вместе со снапшотом, а TSDB Prometheus — это пересоздаваемые метрики, гранулярный point-in-time для них не нужен.

### 14.8 Зависимости от внешних сервисов

- **pve-хост (192.168.1.10)** — node_exporter и Proxmox API (через `monitoring@pve`) как источники метрик.
