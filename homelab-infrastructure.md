...
Отдельный изолированный сегмент для IoT устройств — отдельный SSID `Izolda-Rally-IoT` (2.4 ГГц, WPA2), привязанный к этому сегменту в Keenetic. Роутер сегмента — `192.168.10.1`. DHCP-пул `192.168.10.100–199`. IP адреса определяются динамически.
